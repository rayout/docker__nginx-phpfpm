...
then
   cat /var/spool/cron/crontabs | { cat; echo "* * * * * www-data php /var/www/artisan schedule:run >> /dev/null 2>&1"; } >> /etc/crontab
fi
...
if [ -n "$CRONTAB_JOBS" ]
then
   cat /var/spool/cron/crontabs | { cat; echo -e "$CRONTAB_JOBS" | sed 's/^ *//;s/ *$//'; } >> /etc/crontab
fi
